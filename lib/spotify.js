...
export const getFollowers = async(refresh_token, idUsers) => {
    const { access_token } = await getAccessToken(refresh_token)

    const followedUsers = await fetch(`https://api.spotify.com/v1/me/following/contains?type=user&ids=${idUsers}`, {
        headers: {
            Authorization: `Bearer ${access_token}`
        }
    });

    return followedUsers.json()
}

export const followUser = async(refresh_token, idUser) => {
    const { access_token } = await getAccessToken(refresh_token)

    const response = await fetch(`https://api.spotify.com/v1/me/following?type=user&ids=${idUser}`, {
        method: 'PUT', 
        headers: {
            Authorization: `Bearer ${access_token}`
        }
